...
const promotionEdit = (props) => {
  ...
  useEffect(() => {
    dispatch(editPromotion(slug, props.router.navigate));
  ...
  const selectPromotionState = (state) => state;
  const PromotionpageData = createSelector(selectPromotionState, (state) => ({
    error: state.Message.error,
    messageError: state.Message.messageError,
    item: state.Promotion.item,
  }));
  const { error, messageError, item } = useSelector(PromotionpageData);
  ...
  const validation = useFormik({
    ...
    initialValues: {
      ...
      file: item.image ? [{ uid: item.image, url: item.image }] : [],
    },
    ...
    onSubmit: (values) => {
      ...
      dispatch(updatePromotion(formData, props.router.navigate));
    },
  });

  useEffect(() => {
    if (item.image) {
      setFileList([{ uid: item.image, url: item.image }]);
    }
  }, [item.image]);
